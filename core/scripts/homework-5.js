...
*/
function nowWeather(city, temperature) {
  console.log(
    `Сейчас в ${city} температура  — ${temperature} градусов по Цельсию`,
  );
}

const nowWeatherButton = document.getElementById("now-weather-button");

nowWeatherButton.addEventListener("click", () =>
  nowWeather(
...
const lightSpeedButton = document.getElementById("light-speed-button");

lightSpeedButton.addEventListener("click", () =>
  comparisonSpeed((speed = document.getElementById("speed-input").value)),
);
...
buyProductButton.addEventListener("click", () =>
  buyProduct((budget = document.getElementById("budget-input").value)),
);
...
const sumDigitsButton = document.getElementById("sum-digits-button");

sumDigitsButton.addEventListener("click", () => sumThreeRandomDigits());
